...
def VCMA_Strategy_Screener(tickers_in, df, true_number, catgory):
    global scenario
...
def party(ticker_type, tickers_in, start):
    ...
    for ticker in new_tickers[start:]:
        ...
        # print ("before gogogo")
        # print (ticker)
        if (Run_VCMA_Screener == 1):
            VCMA_Strategy_Screener(ticker, data, run_number, ticker_type)
        if (VCP == 1):
...
VCP = 0
VCP_TEST = 0
VCMA_SCREENER = 0  # 比較成交量與平均成交量，找出成交量成長並且上漲的股票
Run_VCMA_Screener = 1  # 看vcma是否短線大於短線，並且呈現上揚

# 強制寫出plotly，主要用來測試
ForcePLOT = 0  # Force Plot時，Run_VCMA_Screener 也要設成 1
