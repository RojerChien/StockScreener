...
def display_industry_group_data(selected_sector, selected_week_index, industry_groups_in_sector, industry_group_percentages):
    # 创建 DataFrame
    ...
    plt.title(f"{selected_sector} Industry Group Percentage (Week {selected_week_index + 1})")
    ...
    plt.show()
    plt.savefig(f"{selected_sector}_Week_{selected_week_index + 1}.png", bbox_inches='tight')
...
# 转换为 DataFrame
percentage_df = pd.DataFrame(weekly_percentage_by_industry_group).T
percentage_df.columns = [date.strftime('%Y-%m-%d') for date in date_range[:-1]]
percentage_df.index = weekly_percentage_by_industry_group.keys()


# 绘制热力图
plt.figure(figsize=(60, 30))
sns.heatmap(data=percentage_df, annot=True, fmt=".1%", cmap="plasma", linewidths=.5, annot_kws={"size": 4})
plt.title("Industry Group Weekly Volume Percentage")
